...
# Returns nothing.
uncomment() {
    gsed -e "s/^$OPTIONAL_SPACE_RE#[[:space:]]\?//"
}
...
generate_markdown() {
    ...
    printf "%s\n" "$2" | uncomment | gsed -e "s/$SPACE_RE$//" | while IFS='' read -r line; do
        if printf "%s" "$line" | grep -q "^$OPTIONAL_SPACE_RE$NOT_SPACE_RE$SPACE_RE-$SPACE_RE"; then
            ...
            if printf "%s" "$line" | grep -q "^$NOT_SPACE_RE"; then
                ...
                # Careful - BSD gsed always adds a newline
                printf "    * "
                printf "%s" "$line" | gsed "s/^$SPACE_RE//" | tr -d "\n"
            fi
